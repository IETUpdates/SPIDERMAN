...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("പോയി നിൻ്റെ പണി നോക്ക്.", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if (st.status == "creator") or (str(userid) in ADMINS):
                ...
                await query.answer("Thats not for you!! ഇത് നിനക്ക് ഉള്ളതല്ല",show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("I Like Your Smartness, But Don't Be Oversmart 😒 വെളച്ചിൽ എടുക്കല്ലെ.. JOIN ചെയ്യ്.",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🤖 Uᴘᴅᴀᴛᴇs', url='https://t.me/iet_updates'),
            InlineKeyboardButton('👮 Oᴡɴᴇʀ', url='https://t.me/iet_owner')
            ],[
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("I couldn't find anything related to that. Check your spelling in Google.com")
        await asyncio.sleep(8)
        await k.delete()
        return
